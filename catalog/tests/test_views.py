...
from catalog.views import AuthorCreateView
...
class AuthorCreateViewTest(TestCase):
    ...
    def setUp(self):
        ...
        #Создание автора
        self.test_author = Author.objects.create(first_name='John', last_name='Smith', date_of_birth='2000-12-10', date_of_death='2020-12-10')
        #Создание жанра Create genre as a post-step
    ...
    def test_redirects_to_all_author_list_on_success(self):
        ...
        resp = self.client.post(reverse('author_create'),
                                {'date_of_death': valid_date_in_future})
        self.assertRedirects(resp, reverse('authors'))

    def test_form_invalid_author_date_of_birth(self):
        login = self.client.login(username='testuser2', password='12345')
        date_in_past = datetime.date.today() - datetime.timedelta(weeks=48)
        resp = self.client.post(reverse('author_create'), )
        self.assertEqual(resp.status_code, 200)
        self.assertFormError(resp, 'form', 'date_of_birth', 'Invalid date - date in past')

    def test_form_invalid_author_date_of_death(self):
        login = self.client.login(username='testuser2', password='12345')
        invalid_date_in_future = datetime.date.today() + datetime.timedelta(weeks=5)
        resp = self.client.post(reverse('author_create'),
                                {'date_of_death': invalid_date_in_future})
        self.assertEqual(resp.status_code, 200)
        self.assertFormError(resp, 'form', 'date_of_death', 'Invalid date - date more than date of birth')